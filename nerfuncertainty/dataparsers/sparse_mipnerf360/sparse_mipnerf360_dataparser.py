...
@dataclass
class SparseMipNerf360DataParserConfig(NerfstudioDataParserConfig):
    """MipNerf360 dataset parser config"""

    _target: Type = field(default_factory=lambda: SparseMipNerf360)
    """target class to instantiate"""
    proportion_train_images: float = 1.0 # maybe need fraction instead like 5%, 10% (0.05, 0.1, 0.02 etc )
    """How much data to use in percentage"""
    seed_random_split: int = 2024 #Literal['seed1', 'seed2', 'seed3'] = 'seed1'
    """Which random split to use"""

@dataclass
class SparseMipNerf360(Nerfstudio):
    """Nerfstudio DatasetParser"""

    config: SparseMipNerf360DataParserConfig
