...
@dataclass
class NerfOnTheGoDataParserConfig(NerfstudioDataParserConfig):
    """ Nerf-on-the-go dataset parser config"""

    _target: Type = field(default_factory=lambda: NerfOnTheGo)
...
class NerfOnTheGo(Nerfstudio):
    ...
    config: NerfOnTheGoDataParserConfig
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if f"{split}_filenames" in meta:
            ...
            """
            # get train and eval indices based on clutter/extra lists in split.json file
            split_idxs = load_from_json(self.config.data / "split.json")
            assert "clutter" in split_idxs, "clutter idxs not in split.json"
            assert "extra" in split_idxs, "extra idxs not in split.json"

            # NOTE: image_filenames is sorted, so only need to set i_train and i_eval to the lists in split.json 
            i_train = split_idxs['clutter']
            i_eval = split_idxs['extra']
